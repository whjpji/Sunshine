...
public class WeatherProvider extends ContentProvider {
    ...
    static UriMatcher buildUriMatcher() {
        ...
        // Note that the location string is identified as a string,
        // and use "*" to match it. The date is an integer, and use
        // "#" to match it.
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        ...
        // Only notify changes when it exactly changes.
        if (rowsDeleted != 0) {
            getContext().getContentResolver().notifyChange(uri, null);
        }
    ...
    public int update(
        ...
        // Using the Uri matcher to get the uri type.
        final int match = sUriMatcher.match(uri);

        int rowsUpdated = 0;

        // Do update for each type of uri.
        switch (match) {
            case WEATHER:
                // Always normalize date value first.
                normalizeDate(values);
